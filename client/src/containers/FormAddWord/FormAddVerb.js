...
class FormAddVerb extends Component {
  ...
  createVerb = async () => {
    ...
    const body = { ...state }
    delete body.step
    await axios.post("/api/words/create", body)
    // fetch("http://localhost:5000/api/words/create", {
    //   method: "POST",
    //   body,
    // })
  ...
  renderForm = (timeTitles, type) => {
    ...
    timeTitles.map((timeTitle) => {
      let timeInputs = config.filter(
        (input) => input.time === timeTitle && input.type === "input"
      )
      let timeTitles = config.filter(
        (title) => title.time === timeTitle && title.type === "title"
      )
  ...
  render() {
    ...
    if (width < 500) {
      ...
      return (
        <Fragment>
          <div className={classes.FormContainer}>{this.getFormDesktop()}</div>
        </Fragment>
      )
    }
  }
